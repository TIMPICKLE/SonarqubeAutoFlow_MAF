...
用户执行 python cli.py run
        ...
        ▼
┌───────────────────┐
│  Orchestrator     │  初始化: MCPManager + GLM Client + SmellFixAgent
│  .run()           │
└────────┬──────────┘
         │ 创建 WorkflowState (空)
         ▼
┌───────────────────────────────────────────────────────────────┐
│  阶段 1: issue_analysis (确定性)                                │
│  ─────────────────────────────                                  │
│  1. 读取 localJSON/codeSmallList.json → 已处理异味集合           │
│  2. MCPManager.call_sonarqube_api(params)                      │
│     → MCP stdio → npx sonarqube-mcp-server → SonarQube API     │
│  3. 分页遍历，筛选: status=OPEN + author非空 + 未处理 + 非nihao  │
│  4. 选中一条 unprocessed_issue                                  │
│                                                                 │
│  输出: state.smell_data = {key, rule, message, line, component, ...} │
└────────┬──────────────────────────────────────────────────────┘
         │ state.smell_data
         ▼
┌───────────────────────────────────────────────────────────────┐
│  阶段 2: workspace_setup (确定性)                               │
│  ─────────────────────────────                                  │
│  1. 检查 Config.GIT_REPO_PATH 是否存在 .git                    │
│     不存在 → git clone Config.GIT_REPO_URL                     │
│  2. git reset --hard HEAD + git clean -fd (清理残留)            │
│  3. git checkout master + git pull origin master (更新代码)    │
│  4. git checkout -b fix-sonar-{key}-{timestamp} (创建分支)     │
│                                                                 │
│  输出: state.branch_name = "fix-sonar-AXXXX-20260703..."       │
└────────┬──────────────────────────────────────────────────────┘
         │ state.smell_data + state.branch_name
         ▼
┌───────────────────────────────────────────────────────────────┐
│  阶段 3: agent_fix (LLM Agent - MAF Agent)                     │
│  ─────────────────────────────                                  │
│  详见下方 [5.2 Agent 内部数据流]                                 │
│                                                                 │
│  输出: state.fix_solution = {filePath, assignee, description,   │
│         smellKey, line, effort, executionSummary}              │
└────────┬──────────────────────────────────────────────────────┘
         │ state.fix_solution + state.branch_name
         ▼
┌───────────────────────────────────────────────────────────────┐
│  阶段 4: pr_creation (确定性)                                   │
│  ─────────────────────────────                                  │
│  1. 从 fix_solution 构建 PR 参数 (title/description/reviewers)  │
│  2. _ensure_branch_pushed: git add -A + commit + push          │
│  3. MCPManager.call_azure_devops_api("create_pr", params)      │
│     → MCP stdio → npx mcp-server-azure-devops → ADO API        │
│  4. 提取 PR ID → 构建 PR URL                                    │
│  5. 飞书通知:                                                   │
│     - _send_feishu_notification: 群组 webhook (effort统计)     │
│     - _send_message_to_user: 私信 (lark-oapi → 负责人)         │
│                                                                 │
│  输出: state.pr_info = {id, url, title, status}                │
└────────┬──────────────────────────────────────────────────────┘
         │ state.pr_info
         ▼
┌───────────────────────────────────────────────────────────────┐
│  阶段 5: record_keeping (确定性)                                │
│  ─────────────────────────────                                  │
│  1. 构建 record = {key, processedDate, assignee, prUrl, ...}   │
│  2. 读取 localJSON/codeSmallList.json                           │
│  3. 追加新记录 → 写回文件                                       │
│                                                                 │
│  输出: state.completed_steps.append("record_keeping")          │
└────────┬──────────────────────────────────────────────────────┘
         │
         ▼
    流程结束 (成功)
...
```
SmellFixAgent.run(state)
        │
        │  1. 准备上下文
        │     - 解析文件路径: component → file_path → full_file_path
        │     - 查询负责人: email_to_guid.json → assignee_guid
        │     - 构建 user_msg (异味信息 + 文件路径 + 分支名 + 负责人)
        │
        ▼
┌─────────────────────────────────────────────────────────────┐
│  MAF Agent.run(user_msg)                                     │
│  (框架自动处理 ReAct 循环，无需手写)                           │
│                                                              │
│  ┌─────────────────────────────────────────────────────┐    │
│  │  第 1 轮: 思考 → 选工具                              │    │
│  │  LLM 收到: system_prompt + user_msg                  │    │
│  │  LLM 返回: function_call(analyze_smell_type)         │    │
│  │  框架执行: analyze_smell_type(rule, message)         │    │
│  │  结果反馈: {type:"unused", difficulty:"medium"}      │    │
│  └───────────────────────┬─────────────────────────────┘    │
│                          ▼                                    │
│  ┌─────────────────────────────────────────────────────┐    │
│  │  第 2 轮: 思考 → 选工具                              │    │
│  │  LLM 收到: 上一轮工具结果                            │    │
│  │  LLM 返回: function_call(claude_code_fix)            │    │
│  │  框架执行: claude_code_fix(smell_description,         │    │
│  │    file_path, repo_path, branch_name, commit_message)│    │
│  │                                                      │    │
│  │    ┌──────────────────────────────────────┐         │    │
│  │    │  Claude Code CLI 内部执行:            │         │    │
│  │    │  1. 接收 prompt (stdin)              │         │    │
│  │    │  2. 自主浏览代码库                   │         │    │
│  │    │  3. 修改相关文件 (可能多文件)        │         │    │
│  │    │  4. 返回执行结果                     │         │    │
...
│  │    ┌──────────────────────────────────────┐         │    │
│  │    │  fix_tools.py 内部后续处理:          │         │    │
│  │    │  1. _get_modified_files (git diff)   │         │    │
│  │    │  2. _git_commit_and_push             │         │    │
│  │    │     git add -A + commit + push       │         │    │
│  │    │  3. 返回 {success, files_modified,   │         │    │
│  │    │          commit_sha, summary}        │         │    │
│  │    └──────────────────────────────────────┘         │    │
│  │                                                      │    │
│  │  结果反馈: {success:true, commit_sha:"9a44e78"}     │    │
│  └───────────────────────┬─────────────────────────────┘    │
│                          ▼                                    │
│  ┌─────────────────────────────────────────────────────┐    │
│  │  第 3 轮: 思考 → 完成                                │    │
│  │  LLM 收到: claude_code_fix 成功结果                  │    │
│  │  LLM 返回: 文本总结 (无 function_call)               │    │
│  │  循环结束                                            │    │
│  └─────────────────────────────────────────────────────┘    │
│                                                              │
│  返回: AgentResponse (response.text = 总结文本)              │
└──────────────────────────┬──────────────────────────────────┘
                           │
                           ▼
  2. 解析 Agent 响应 → fix_result
     - 从文本中判断 success/failed
  3. 构建 state.fix_solution
  4. 更新 state.current_step = "pr_creation"
```
